...
const definitions={codex:[['CODEX_HOME','CODEX_DIR'],'.codex'],claude:[['CLAUDE_CONFIG_DIR','CLAUDE_HOME'],'.claude'],grok:[['GROK_HOME','GROK_DIR'],'.grok'],deepseek:[['DEEPSEEK_HOME','DEEPSEEK_DIR'],'.deepseek'],glm53:[['GLM_HOME','ZHIPU_HOME'],'.glm'],gemini:[['GEMINI_HOME','GEMINI_DIR'],'.gemini']};
function detectDirectory(seat,{env=process.env,home=os.homedir()}={}){
 ...
 if(seat==='deepseek'&&env.HERMES_HOME){add(env.HERMES_HOME,'HERMES_HOME','hermes');return candidates[0];}
 if(seat==='glm53'&&env.ZCODE_HOME){add(env.ZCODE_HOME,'ZCODE_HOME','zcode');return candidates[0];}
 add(path.join(home,def[1]),'约定配置目录');
 if(seat==='deepseek')add(path.join(home,'.hermes'),'Hermes 配置目录','hermes');
